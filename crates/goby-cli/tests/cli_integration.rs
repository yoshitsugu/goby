...
#[test]
fn run_command_executes_read_lines_program_with_runtime_stdin() {
    let root = repo_root();
    let sandbox = TempDirGuard::new("run_read_lines_interpreter_bridge");
    let input = sandbox.join("read_lines.gb");
    fs::write(
        &input,
        r#"
import goby/list ( each )
...
"#,
    ...
    assert!(
        stdout.contains("hogehoge!\nfugafuga!\npiyo!"),
        "expected newline-normalized read_lines output, stdout: {}",
        stdout
    );
}
...
fn unresolved_name_error_output_matches_fixture() {
    ...
    let expected_path =
        root.join("crates/goby-cli/tests/fixtures/unresolved_name_error_expected.txt");
    assert!(
        root.join(input).exists(),
        "fixture input must exist at {}",
        input
    ...
    assert!(
        !output.status.success(),
        "unresolved-name fixture should fail"
    );
    let actual = String::from_utf8_lossy(&output.stderr);
    assert_eq!(
        actual.trim_end_matches('\n'),
...
fn import_typo_error_output_matches_fixture() {
    ...
    let expected_path = root.join("crates/goby-cli/tests/fixtures/import_typo_error_expected.txt");
    assert!(
        root.join(input).exists(),
        "fixture input must exist at {}",
        input
    ...
    assert!(!output.status.success(), "import-typo fixture should fail");
    let actual = String::from_utf8_lossy(&output.stderr);
    assert_eq!(
        actual.trim_end_matches('\n'),
        expected_raw.trim_end_matches('\n'),
        "stderr does not match import_typo_error_expected.txt"
    );
}
