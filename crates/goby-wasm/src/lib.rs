...
impl<'m> RuntimeOutputResolver<'m> {
    ...
    fn try_eval_imported_decl_call_as_value(
        &mut self,
        expr: &Expr,
    ...
    fn try_execute_imported_decl_call_as_side_effect(
        &mut self,
        expr: &Expr,
        caller_locals: &RuntimeLocals,
        caller_callables: &HashMap<String, IntCallable>,
    ...
    fn eval_decl_call_chain_as_value(
        &mut self,
        fn_name: &str,
        args: &[&Expr],
        caller_locals: &RuntimeLocals,
    ...
    fn execute_unit_ast_stmt_sequence(
        &mut self,
        stmts: &[Stmt],
        locals: &mut RuntimeLocals,
        callables: &mut HashMap<String, IntCallable>,
    ...
    fn current_handler_resume_value(&self, token_idx: usize) -> Option<RuntimeValue> {
    ...
    fn handler_has_suspended_cont(&self, token_idx: usize) -> bool {
        match self.execution_mode {
            lower::EffectExecutionMode::PortableFallback => self
                .resume_tokens
                .get(token_idx)
    ...
    fn push_optimized_resume_token_for_handler(&mut self, take_caller_cont: bool) -> usize {
        let caller_cont = if take_caller_cont {
            self.pending_caller_cont_stack
                .last_mut()
                .and_then(|s| s.take())
    ...
    fn begin_handler_continuation_bridge(&mut self, take_caller_cont: bool) -> usize {
        match self.execution_mode {
            lower::EffectExecutionMode::PortableFallback => {
    ...
    fn resume_through_active_continuation_fallback_out(
        ...
        // token.cont was None; no AST frame either (pending_value_continuations always empty).
        // Treat as a no-op frame: return the resumed value directly and mark as Resumed.
        token.continuation.consumed = true;
        if let Some(token) = self.current_resume_token_mut() {
    ...
    fn resolve_imported_runtime_decl(&self, head: &DirectCallHead) -> Option<RuntimeDeclInfo> {
        match head {
    ...
    fn runtime_decl_info_from_decl(
